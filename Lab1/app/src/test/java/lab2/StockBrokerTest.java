...
public class StockBrokerTest
{
    ...
    public void verify_no_more_interaction()
    {
        ...
        verify(portfolio).getAvgPrice(Matchers.eq(noStock));
    ...
    public void argument_matcher()
    {
        ...
        when(marketWatcher.getQuote(Matchers.argThat(new BlueChipStockMatcher())))
                .thenReturn(blueChipStock);
        when(marketWatcher.getQuote(Matchers.argThat(new OtherStockMatcher())))
    ...
    {
        when(portfolio.getAvgPrice(Matchers.isA(Stock.class))).thenThrow(
    ...
    {
        stockMap.clear();
        doAnswer(new BuyAnswer()).when(portfolio).buy(Matchers.isA(Stock.class));
        ...
        Assert.assertEquals(new BigDecimal("11"), portfolio.getCurrentValue());
    }
    ...
    public void doReturn_usage() throws Exception
    {
        ...
        Assert.assertEquals("now reachable", spy.get(0));
    }
    ...
    public void argument_captor() throws Exception
    {
        ...
        Assert.assertEquals("A", arg.getValue());
        ...
        Assert.assertEquals("A", arg0.getValue().getSymbol());
        Assert.assertEquals(10, arg1.getValue().intValue());
    }
    ...
    public void inorder() throws Exception
    {
        ...
        inOrder.verify(portfolio).getAvgPrice(Matchers.isA(Stock.class));
        inOrder.verify(portfolio).getCurrentValue();
        inOrder.verify(marketWatcher).getQuote(anyString());
        inOrder.verify(portfolio).buy(Matchers.isA(Stock.class));
    }
    ...
    public void resetMock() throws Exception
    {
        ...
        when(portfolio.getAvgPrice(Matchers.eq(aCorp))).thenReturn(BigDecimal.ONE);
        Assert.assertNotNull(portfolio.getAvgPrice(aCorp));

        Mockito.reset(portfolio);
        //Resets the stub, so getAvgPrice returns NULL
        Assert.assertNull(portfolio.getAvgPrice(aCorp));
    }

    @Test
    public void access_global_mock() throws Exception
    {
        Assert.assertEquals(BigDecimal.ONE, globalStock.getPrice());
    }
    ...
    public void mocking_details() throws Exception
    {
        ...
        Assert.assertTrue(Mockito.mockingDetails(pf1).isMock());

        Stock myStock = new Stock(null, null, null);
        Stock spy = spy(myStock);
        Assert.assertTrue(Mockito.mockingDetails(spy).isSpy());
